...
# Apache
sudo sed "s@%%DOCROOT%%@$PWD/wordpress@g" .devcontainer/000-default.conf | sudo tee /etc/apache2/sites-available/000-default.conf >/dev/null
sudo a2enmod rewrite
sudo service apache2 start

LOCALE="de_DE"
echo memory_limit=256M | sudo tee -a /usr/local/etc/php/conf.d/xmemory.ini
